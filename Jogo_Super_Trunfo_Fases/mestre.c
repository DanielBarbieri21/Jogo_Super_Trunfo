...
// Estrutura para armazenar widgets e estado do jogo
typedef struct {
    GtkWidget *window;
    ...
    GtkWidget *label_placar_j1, *label_placar_j2;
    int carta_atual; // Índice da carta sendo editada
    int pontos_j1, pontos_j2;
    int turno; // 0 para Jogador 1, 1 para Jogador 2
} AppWidgets;
...
void atualizar_placar() {
    char placar_j1[20], placar_j2[20];
    snprintf(placar_j1, 20, "Jogador 1: %d", app.pontos_j1);
    snprintf(placar_j2, 20, "Jogador 2: %d", app.pontos_j2);
    gtk_label_set_text(GTK_LABEL(app.label_placar_j1), placar_j1);
    gtk_label_set_text(GTK_LABEL(app.label_placar_j2), placar_j2);
}
...
void on_cadastrar_clicked(GtkWidget *button, gpointer data) {
    ...
    // Validação simples
    if (strlen(estado) == 0 || atoll(pop_str) <= 0 || atof(pib_str) <= 0 || atof(area_str) <= 0 || atoi(pontos_str) < 0) {
        GtkWidget *dialog = gtk_message_dialog_new(GTK_WINDOW(app.window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "Valores inválidos! Certifique-se de que todos os campos estão preenchidos corretamente.");
...
void on_compare_clicked(GtkWidget *button, gpointer data) {
    ...
    char resultado[200];
    if (attr1 > attr2) {
        snprintf(resultado, 200, "Comparando %s:\n%s: %.2lf\n%s: %.2lf\n%s vence! (+1 para Jogador %d)", 
                 atributo, cartas[idx1].estado, attr1, cartas[idx2].estado, attr2, cartas[idx1].estado, app.turno + 1);
        if (app.turno == 0) app.pontos_j1++;
        else app.pontos_j2++;
    } else if (attr2 > attr1) {
        snprintf(resultado, 200, "Comparando %s:\n%s: %.2lf\n%s: %.2lf\n%s vence! (+1 para Jogador %d)", 
                 atributo, cartas[idx1].estado, attr1, cartas[idx2].estado, attr2, cartas[idx2].estado, (1 - app.turno) + 1);
        if (app.turno == 0) app.pontos_j2++;
        else app.pontos_j1++;
    } else {
        snprintf(resultado, 200, "Comparando %s:\n%s: %.2lf\n%s: %.2lf\nEmpate! Ninguém pontua.", 
                 atributo, cartas[idx1].estado, attr1, cartas[idx2].estado, attr2);
    }
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_title(GTK_WINDOW(app.window), "Super Trunfo - Nível Mestre");
    ...
    // Placar
    app.label_placar_j1 = gtk_label_new("Jogador 1: 0");
    app.label_placar_j2 = gtk_label_new("Jogador 2: 0");
    gtk_grid_attach(GTK_GRID(grid), app.label_placar_j1, 0, 12, 1, 1);
    gtk_grid_attach(GTK_GRID(grid), app.label_placar_j2, 1, 12, 1, 1);

    // Inicializar pontos e turno
    app.pontos_j1 = 0;
    app.pontos_j2 = 0;
    app.turno = 0; // Começa com Jogador 1
